...
import {
  createReorderCriticalSection,
  keyboardReorderIntent,
  persistReorderCandidate,
} from './note-order-interaction.js';
...
const elements = {
  ...
  orderHint: document.querySelector('#note-order-hint'),
...
const reorderCriticalSection = createReorderCriticalSection(locked => {
  elements.workspace.inert = locked;
});
...
function displayNotes() {
  return orderedNotes(state);
}

function renderList() {
  const searchActive = Boolean(elements.search.value.trim());
  const filtered = filterNotes(displayNotes(), elements.search.value);
  const rows = filtered.map(note => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = `note-row${note.id === state.activeNoteId ? ' is-active' : ''}`;
    button.dataset.noteId = note.id;
    button.draggable = !searchActive;
    if (!searchActive) button.setAttribute('aria-keyshortcuts', 'Alt+ArrowUp Alt+ArrowDown');
    button.title = searchActive ? note.title : `${note.title} · 拖动或 Option/Alt + ↑/↓ 排序`;

    const grip = document.createElement('span');
    grip.className = 'note-row-grip';
    grip.setAttribute('aria-hidden', 'true');
    grip.textContent = '⠿';
    const copy = document.createElement('span');
    copy.className = 'note-row-copy';
    ...
    button.append(grip, copy);
    return button;
  });
  elements.list.replaceChildren(...rows);
  elements.empty.hidden = rows.length !== 0;
  elements.orderHint.hidden = searchActive;
...
function renderNavigation() {
  ...
  elements.next.disabled = index < 0 || index >= notes.length - 1;
}
...
async function stepNote(delta) {
  const notes = displayNotes();
  const index = notes.findIndex(note => note.id === state.activeNoteId);
  const next = notes[index + Math.sign(Number(delta) || 0)];
  if (next) await switchNote(next.id);
}

function clearDropFeedback() {
  for (const row of elements.list.querySelectorAll('.is-drop-before, .is-drop-after')) {
...
function focusNoteRow(noteId) {
  const row = [...elements.list.children].find(item => item.dataset.noteId === noteId);
  row?.focus({ preventScroll: true });
}

async function applyReorder(movedId, targetId, placement, { restoreFocus = false } = {}) {
  if (elements.search.value.trim() || movedId === targetId) return false;
  let outcome;
  try {
    outcome = await reorderCriticalSection.run(async () => {
      await saver.flush();
      const visibleOrder = displayNotes();
      if (!visibleOrder.some(note => note.id === movedId) || !visibleOrder.some(note => note.id === targetId)) return false;
      const currentState = state;
      const candidateState = reorderNotes({ ...currentState, notes: visibleOrder }, movedId, targetId, placement);
      const result = await persistReorderCandidate(
        currentState,
        candidateState,
        nextState => repository.saveState(nextState),
      );
      if (!result.saved) {
        setStatus('保存失败');
        return false;
  ...
  if (outcome.value && restoreFocus) focusNoteRow(movedId);
...
elements.list.addEventListener('dragstart', event => {
  const row = event.target.closest('[data-note-id]');
  if (!row?.draggable || elements.search.value.trim()) {
    event.preventDefault();
    return;
  }
...
elements.list.addEventListener('dragover', event => {
  const row = event.target.closest('[data-note-id]');
  if (!draggedNoteId || !row?.draggable || row.dataset.noteId === draggedNoteId || elements.search.value.trim()) {
    clearDropFeedback();
    return;
  }
  event.preventDefault();
...
elements.list.addEventListener('drop', async event => {
  const row = event.target.closest('[data-note-id]');
  const movedId = draggedNoteId || event.dataTransfer?.getData('text/plain');
  if (!row || !movedId || row.dataset.noteId === movedId || elements.search.value.trim()) {
    clearDropFeedback();
    return;
  }
  event.preventDefault();
  const placement = dropPlacement(row, event);
  clearDropFeedback();
  await applyReorder(movedId, row.dataset.noteId, placement);
});
elements.list.addEventListener('dragend', event => {
  event.target.closest('[data-note-id]')?.classList.remove('is-dragging');
  draggedNoteId = null;
  clearDropFeedback();
});
elements.list.addEventListener('keydown', async event => {
  ...
  const row = event.target.closest('[data-note-id]');
  if (!row || elements.search.value.trim()) return;
  event.preventDefault();
  const intent = keyboardReorderIntent(displayNotes(), row.dataset.noteId, event.key === 'ArrowUp' ? 'up' : 'down');
  if (!intent) return;
  await applyReorder(row.dataset.noteId, intent.targetId, intent.placement, { restoreFocus: true });
});
